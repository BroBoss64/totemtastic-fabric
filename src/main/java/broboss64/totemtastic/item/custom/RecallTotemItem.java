...
public class RecallTotemItem extends Item {
        ...
        if (!world.isClient()) {
            if (hand == Hand.MAIN_HAND) {
                ...
                if (user.isSneaking()) {
                    ...
                    TotemtasticItemUtils.createPositionLinkedTotem(user.getBlockPos(), world, user);
    ...
    public void usageTick(World world, LivingEntity user, ItemStack stack, int remainingUseTicks) {
        if (!world.isClient() && user instanceof PlayerEntity) {
            ...
            BlockPos targetBlock = TotemtasticItemUtils.fetchBlockPosFromItemStack(stack);
            if (remainingUseTicks >= 1 && !user.isSneaking()) {
                ...
                serverWorld.spawnParticles(ParticleTypes.PORTAL, user.getX(), user.getY() + 1, user.getZ(),
                        2, 0.25, 0.5, 0.25, 0.3);
                if (targetBlock != null) {
                    serverWorld.spawnParticles(ParticleTypes.PORTAL, targetBlock.getX(), targetBlock.getY() + 1, targetBlock.getZ(),
                            2, 0.25, 0.5, 0.25, 0.3);
                }
    ...
    private void returnHome(World world, PlayerEntity user, ItemStack stack) {
        if (!world.isClient()) {
            BlockPos targetPos = TotemtasticItemUtils.fetchBlockPosFromItemStack(stack);
            ServerWorld targetWorld = TotemtasticItemUtils.fetchDimensionFromItemStack(stack, world.getServer());
            ...
            if (targetWorld != null && targetPos != null) {
                ...
                targetWorld.playSoundAtBlockCenter(targetPos, SoundEvents.BLOCK_PORTAL_TRAVEL, SoundCategory.PLAYERS, 0.6f, 1, true);
